...
function createEventTypeItemTemplate(eventTypesList, selecteEeventType) {
  let eventTypeItemTemplate = '';
  for (let i = 0; i < eventTypesList.length; i++) {
    const selectedEventTypeAttribute = selecteEeventType.toLowerCase() === eventTypesList[i].toLowerCase() ? 'checked' : '';
...
function createEditEventFormTemplate(event) {
  const {eventData: { basePrice, dateFrom, dateTo, type, }, city: { name: cityName, description: cityDescription}, selectedOffers, offers, citiesList } = event;
